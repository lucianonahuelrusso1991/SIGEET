...
def dashboard(request):
    ...
    tiene_alumno = hasattr(request.user, 'perfil_alumno')
    tiene_docente = hasattr(request.user, 'perfil_docente')
    es_admin = request.user.is_staff or request.user.is_superuser

    if tiene_alumno and tiene_docente and not es_admin:
        rol_activo = request.session.get('rol_activo')
        if not rol_activo:
            return redirect('seleccionar_perfil')
    else:
        if tiene_alumno and not es_admin:
            rol_activo = 'alumno'
        elif tiene_docente and not es_admin:
            rol_activo = 'docente'
        else:
            rol_activo = 'admin'

    # Bifurcación para panel de alumno
    if rol_activo == 'alumno':
        alumno = request.user.perfil_alumno
        
        # Obtener todas las carreras inscriptas
        inscripciones_carreras = alumno.inscripciones_carreras.all().select_related('plan')
...
def alta_comision(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            messages.success(request, f"🎉 Comisión para {materia.nombre} abierta con éxito. (Aula de Classroom no creada automáticamente).")
...
def cargar_notas(request, comision_id):
    ...
    # Optimización N+1: Calcular faltas para todos los alumnos de la comisión en una sola pasada
    asistencias_comision = RegistroAsistencia.objects.filter(planilla__comision=comision).select_related('alumno')
    faltas_por_alumno = {}
    for r in asistencias_comision:
        if r.alumno_id not in faltas_por_alumno:
            faltas_por_alumno[r.alumno_id] = Decimal('0.0')
        if r.estado == 'A':
            faltas_por_alumno[r.alumno_id] += Decimal('1.0')
        elif r.estado == 'T':
            faltas_por_alumno[r.alumno_id] += Decimal('0.5')
    ...
    if request.method == 'POST':
        ...
        for insc in inscripciones:
            ...
            todas_las_notas = list(insc.notas.all()) # Usar cache de prefetch
            ...
            if nota_valor:
                ...
                # Validación Backend Estricta para Recuperatorio
                if instancia_post == 'Recuperatorio':
                    parc_aplazos = sum(1 for n in todas_las_notas if 'Parcial' in n.instancia and n.valor_nota < 4)
                    faltas_acum = float(faltas_por_alumno.get(insc.alumno_id, Decimal('0.0')))
                    es_anual = comision.cuatrimestre == 'AN'
                    limite_faltas = 8 if es_anual else 4
                    
                    if parc_aplazos > 1:
                        messages.warning(request, f"{insc.alumno.apellido} tiene más de 1 aplazo, no puede rendir Recuperatorio.")
                        continue
                    if parc_aplazos == 0 and faltas_acum <= limite_faltas:
                        messages.warning(request, f"{insc.alumno.apellido} no requiere Recuperatorio.")
                        continue
                        
                nueva_nota = Nota.objects.create(
                    inscripcion=insc,
                    instancia=instancia_post,
                    valor_nota=nota_valor_int
                )
                todas_las_notas.append(nueva_nota) # Mantener la lista actualizada para calcular estado
                notas_guardadas += 1
            
            # Si se solicita cerrar la cursada, se calcula el estado automáticamente
            if cerrar_cursada:
                if len(todas_las_notas) > 0:
                    if instancia_post == 'Nota Final' and nota_valor:
                        if nota_valor_int >= 7:
                            nuevo_estado = 'PROM' if comision.tipo_aprobacion == 'PROM' else 'APR'
                        elif nota_valor_int >= 4:
                            nuevo_estado = 'APR'
                        else:
                            nuevo_estado = 'LIB'
                    else:
                        parciales_aplazados = sum(1 for n in todas_las_notas if 'Parcial' in n.instancia and n.valor_nota < 4)
                        ...
                        if parciales_aplazados > 1:
                            ...
                            notas_recup = [n.valor_nota for n in todas_las_notas if 'Recuperatorio' in n.instancia]
                            aprobo_recup = any(nota >= 4 for nota in notas_recup)
                            
                            if requiere_recuperatorio and not aprobo_recup:
                                nuevo_estado = 'LIB'
                            else:
                                # Notas validas para promedio (se excluye el recup porque reemplaza una nota)
                                notas_validas = [n.valor_nota for n in todas_las_notas if 'Recuperatorio' not in n.instancia and n.instancia != 'Nota Final']
                                        
                                if requiere_recuperatorio and notas_recup:
                                    if parciales_aplazados == 1:
                                        # Reemplazar el aplazo por el recuperatorio
                                        for i in range(len(notas_validas)):
                                            if notas_validas[i] < 4:
                                                notas_validas[i] = max(notas_recup)
                                                break
                                    else:
                                        # Fue por faltas, se añade al promedio
                                        notas_validas.append(max(notas_recup))
                                
                                if notas_validas:
                                    promedio = sum(notas_validas) / len(notas_validas)
                                    todas_mayor_o_igual_7 = all(nota >= 7 for nota in notas_validas)
                                else:
                                    promedio = 0
                                    todas_mayor_o_igual_7 = False
                                    
                                if todas_mayor_o_igual_7 and notas_validas:
                                    nuevo_estado = 'PROM' if comision.tipo_aprobacion == 'PROM' else 'APR'
                                elif promedio >= 4:
                                    nuevo_estado = 'APR'
                                else:
        ...
        if cerrar_cursada:
            ...
            comision.save()
            messages.success(request, f"¡Notas guardadas y cursada CERRADA automáticamente para los alumnos evaluados!")
        else:
            messages.success(request, f"¡Se guardaron {notas_guardadas} notas para la instancia '{instancia_post}'!")
            
        # Refrescar para la UI post-guardado
        inscripciones = list(Inscripcion.objects.filter(comision=comision).prefetch_related('notas', 'alumno__asistencias'))
...
def seleccionar_perfil(request):
    return render(request, 'gestion/seleccionar_perfil.html')
